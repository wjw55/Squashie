...
const comparisonRows: {
  ...
  {
    label: 'Suitable for',
    value: (community) => community.suitableFor,
  },
  {
    label: 'Membership',
    value: (community) => community.eligibility,
  },
  {
    label: 'Indicative cost',
    value: (community) => community.indicativeCost,
  },
  {
    label: 'Joining fee',
    value: (community) => community.joiningFee,
  },
  {
    label: 'Recurring fee',
    value: (community) => community.recurringFee,
  },
  {
    label: 'Court fee',
    value: (community) => community.courtFee,
  },
  {
    label: 'Guest access',
    value: (community) => community.guestFee,
  },
  {
    label: 'Courts',
    value: (community) => community.courtCount,
  },
  {
    label: 'Social play',
    value: (community) => community.socialPlay,
  },
  {
    label: 'Training',
    value: (community) => community.trainingSummary,
  },
  {
    label: 'Levels',
    value: (community) => community.levels.join(', '),
  },
];
...
export function DiscoveryClient({ communities }: { communities: Community[] }) {
  ...
  useEffect(() => {
    ...
    const params = serializeDiscoveryParams(
      { query, region, category, access, level, trainingOnly },
      compared,
    );
      ...
      <section className="border-b border-border/70 bg-[linear-gradient(180deg,#f1f7f1_0%,#fbfcf8_100%)]">
        <div className="mx-auto max-w-7xl px-4 py-9 sm:px-6 sm:py-12 lg:px-8">
          ...
          <div className="mt-7 max-w-5xl rounded-2xl border border-border bg-card p-3 shadow-[0_18px_60px_rgba(18,72,50,0.08)] sm:p-4">
            ...
            >
              <button
                onClick={() => quickFilter('campus')}
                className="filter-chip"
              >
                Near campus
              </button>
              <button
                onClick={() => quickFilter('public')}
                className="filter-chip"
              >
                Open to public
              </button>
              <button
                onClick={() => quickFilter('training')}
                className="filter-chip"
              >
                Structured training
              </button>
              <button
                onClick={() => quickFilter('beginner')}
                className="filter-chip"
              >
                Beginner friendly
              </button>
                ...
                <div className="flex items-start gap-3 rounded-xl bg-muted/55 p-3 text-sm">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setTrainingOnly(checked === true)
                    }
                    className="mt-0.5"
                  />
                  <label htmlFor="training-only" className="cursor-pointer">
            ...
            {filtered.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-card px-5 py-14 text-center">
                ...
                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">
                  Try a nearby region, remove one filter, or browse all
                  communities.
                </p>
                <Button
                  onClick={resetFilters}
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm font-semibold">
                {compared.length}{' '}
                {compared.length === 1 ? 'community' : 'communities'} selected
              </p>
            ...
            <div className="grid gap-3 p-4 md:hidden">
              {comparedCommunities.map((community) => (
                <MobileComparisonCard
                  key={community.slug}
                  community={community}
                  onRemove={() => toggleCompare(community.slug)}
                />
              ))}
            </div>
                  ...
                  {comparedCommunities.map((community) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button
                          type="button"
                          onClick={() => toggleCompare(community.slug)}
